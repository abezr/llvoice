...
System context, container diagrams, and specs for the low-latency voice chatbot MVP (WebSocket-first, Opus 20ms, server-side VAD/ASR/LLM/TTS pipeline).

## One-Page Summary
- Transport: WebSocket-first for control and media; Opus 48kHz mono 20ms frames; binary framing with small headers. WebRTC media is a later optimization.
...
flowchart TD
  ...
  subgraph Edge ["Edge Gateway (Node TS)"]
    ...
    auth["Auth and Rate Limit (JWT + Redis bucket)"]
...
### Binary Framing for Audio (WebSocket)
- Uplink `audio.chunk`:
  - Header (5 bytes):
    - byte0: magic `0xA1`
  ...
  - Payload: Opus frame 20ms at 48kHz mono.
- Downlink `tts.chunk`:
  - Header (5 bytes):
    - byte0: magic `0xB1`
    ...
    - byte2-3: seq (u16 big-endian)
    - byte4: durationMs
  - Payload: Opus 20ms at 48kHz mono.
...
| Network RTT (same region)     | 40-60       | Regional affinity; TLS resume; WebSocket keepalive                              |
...
- Streaming: WebSocket binary frames per 20ms with header; `audio.eos` on push-to-talk release or client VAD end; heartbeats.
- Playback: WebAudio plus wasm Opus decode; 2-3 frame buffer (~40-60ms); support `barge` by clearing buffer and pausing.
- UI states: connecting, listening (PTT/VAD), thinking, speaking, error/retry; show partial transcripts.
- Error UX: toast and retry; degrade to text-only if TTS fails; display timeouts.
...
- VAD/ASR: Rust/Go VAD (WebRTC VAD or Silero); streaming ASR vendor (low-latency tier).
...
- Auth: JWT/OIDC with tenant claim; WebSocket over TLS; key rotation.
...
- Audit: Postgres audit table (session start/end/errors/admin actions); log admin access; encryption at rest and in transit.

## 2-Week MVP Scope
- Ship: WebSocket media and control; Opus 20ms; server VAD; streaming ASR+LLM+TTS; barge-in cancel; Redis FSM and rate limits; Postgres minimal schema; metrics for TTFA; basic error fallbacks to text.
...
- Tests: unit (WebSocket framing, Redis rate limiter, FSM transitions); integration (happy path, barge-in, timeouts for ASR/LLM/TTS); load (200-500 sessions); latency (loopback TTFA breakdown).
